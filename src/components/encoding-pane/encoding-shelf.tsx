...
class EncodingShelfBase extends React.PureComponent<EncodingShelfProps, EncodingShelfState> {
  ...
  public render() {
    ...
    const popupComponent = this.state.customizerIsOpened && (
      <FieldCustomizer
        shelfId={id}
        fieldDef={fieldDef}
      ...
      <div styleName={isWildcardShelf ? 'wildcard-shelf' : 'encoding-shelf'}>
        <div styleName="shelf-label">
          <TetherComponentWrapper
            ...
            closePopup={this.closeCustomizer.bind(this)}
            popupIsOpened={this.state.customizerIsOpened}
          >
            {targetComponent}
            {popupComponent}
          </TetherComponentWrapper>
