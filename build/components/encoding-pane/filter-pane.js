    ...
    FilterPaneBase.prototype.renderFilterShelf = function (filter, index) {
        ...
        if (filter_1.isRangeFilter(filter)) {
            if (fieldDef.type === expandedtype_1.ExpandedType.TEMPORAL) {
                if (timeUnit) {
                    domain = filter_2.getDefaultRange(domain, timeUnit);
                }
                else {
                    // schema.domain returns Date[] for temporal type, we need to convert it to DateTime[]
                    domain = [filter_2.convertToDateTimeObject(Number(domain[0])), filter_2.convertToDateTimeObject(Number(domain[1]))];
                }
            }
        ...
        return (React.createElement("div", { styleName: 'filter-shelf', key: index },
            React.createElement(index_1.Field, { draggable: false, fieldDef: fieldDef, caretShow: true, isPill: true, onRemove: this.filterRemove.bind(this, index), popupComponent: popupComponent, handleAction: handleAction }),
            filterComponent));
    };
...
var filterShelfTarget = {
    drop: function (props, monitor) {
        ...
        if (filter_2.containsFilter(props.filters, filter)) {
