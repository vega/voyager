...
var RangeFilterShelfBase = (function (_super) {
    ...
    RangeFilterShelfBase.prototype.render = function () {
        ...
        if (renderDateTimePicker) {
            ...
            minInput = this.renderDateTimePicker(new Date(filter_2.convertToTimestamp(filter.range[0])), 'min');
            maxInput = this.renderDateTimePicker(new Date(filter_2.convertToTimestamp(filter.range[1])), 'max');
            currMin = filter_2.convertToTimestamp(filter.range[0]);
            currMax = filter_2.convertToTimestamp(filter.range[1]);
            lowerBound = Math.floor(filter_2.convertToTimestamp(domain[0]));
            upperBound = Math.ceil(filter_2.convertToTimestamp(domain[1]));
        }
        else {
    ...
    RangeFilterShelfBase.prototype.filterModifyExtent = function (input) {
        ...
        if (this.props.renderDateTimePicker) {
            range = [filter_2.convertToDateTimeObject(input[0]), filter_2.convertToDateTimeObject(input[1])];
        }
        else {
            range = input;
        }
    ...
    RangeFilterShelfBase.prototype.filterModifyMaxBound = function (e) {
        ...
        var _a = this.props, handleAction = _a.handleAction, index = _a.index;
        if (this.props.renderDateTimePicker) {
            maxBound = filter_2.convertToDateTimeObject(maxBound);
        }
        handleAction({
    ...
    RangeFilterShelfBase.prototype.filterModifyMinBound = function (e) {
        ...
        if (renderDateTimePicker) {
            minBound = filter_2.convertToDateTimeObject(minBound);
        }
        handleAction({
