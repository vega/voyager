...
function shelfSpecReducer(shelfSpec, action, schema) {
    ...
    switch (action.type) {
        ...
        case shelf_1.SHELF_FIELD_ADD: {
            var _c = action.payload, shelfId = _c.shelfId, fieldDef = _c.fieldDef, replace = _c.replace;
...
function addEncoding(shelf, shelfId, fieldDef, replace) {
    if (!fieldDef) {
        ...
        if (replace && shelf.anyEncodings[index]) {
            return __assign({}, shelf, { anyEncodings: util_2.modifyItemInArray(shelf.anyEncodings, index, function () {
                    return __assign({ channel: wildcard_1.SHORT_WILDCARD }, fieldDef);
                }) });
        }
        // insert between two pills (not replace!)
        return __assign({}, shelf, { anyEncodings: util_2.insertItemToArray(shelf.anyEncodings, index, __assign({ channel: wildcard_1.SHORT_WILDCARD }, fieldDef)) });
    }
    else {
...
function modifyEncoding(shelf, shelfId, modifier) {
    if (models_1.isWildcardChannelId(shelfId)) {
        return __assign({}, shelf, { anyEncodings: util_2.modifyItemInArray(shelf.anyEncodings, shelfId.index, modifier) });
    }
    else {
...
function removeEncoding(shelf, shelfId) {
    if (models_1.isWildcardChannelId(shelfId)) {
        var index = shelfId.index;
        var _c = util_2.removeItemFromArray(shelf.anyEncodings, index), anyEncodings = _c.array, item = _c.item;
        if (item) {
