...
exports.summaries = {
    ...
    filterGroupBy: common_1.GROUP_BY_SIMILAR_DATA_AND_TRANSFORM,
        ...
        var newSpec = __assign({}, spec, { mark: common_1.makeWildcard(spec.mark), encodings: spec.encodings.reduce(function (encodings, encQ) {
                if (encoding_1.isFieldQuery(encQ)) {
                    if (wildcard_1.isWildcard(encQ.type)) {
                        ...
                        return encodings.concat(__assign({}, encQ, { aggregate: common_1.makeWildcard(encQ.aggregate), bin: common_1.makeWildcard(encQ.bin), timeUnit: common_1.makeWildcard(encQ.timeUnit) }));
                    }
                    else {
                        switch (encQ.type) {
                            case 'quantitative':
