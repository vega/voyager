...
function shelfSpecReducer(shelfSpec, action) {
    ...
    switch (action.type) {
        ...
        case shelf_1.SPEC_VALUE_CHANGE: {
            var _k = action.payload, shelfId = _k.shelfId, valueDef = _k.valueDef;
            if (models_1.isWildcardChannelId(shelfId)) {
                throw Error('constant value cannot be assigned to a wildcard channel');
            }
            else {
                return __assign({}, shelfSpec, { encoding: __assign({}, shelfSpec.encoding, (_l = {}, _l[shelfId.channel] = { value: valueDef.value }, _l)) });
            }
        }
    }
    return shelfSpec;
