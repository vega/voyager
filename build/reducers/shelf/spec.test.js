...
describe('reducers/shelf/spec', function () {
    describe(shelf_1.SPEC_CLEAR, function () {
        it('should return DEFAULT_SHELF_UNIT_SPEC', function () {
            ...
            }, { type: shelf_1.SPEC_CLEAR }, schema)).toBe(models_1.DEFAULT_SHELF_UNIT_SPEC);
        });
    });
    ...
    describe(spec_1.SPEC_FUNCTION_ENABLE_WILDCARD, function () {
        ...
        it('shoud correctly change undefined to wildcard', function () {
            var shelfSpec = spec_2.shelfSpecReducer(__assign({}, models_1.DEFAULT_SHELF_UNIT_SPEC, { encoding: {
                    x: { field: 'a', type: 'quantitative' }
                } }), {
                type: spec_1.SPEC_FUNCTION_ENABLE_WILDCARD,
    ...
    describe(spec_1.SPEC_LOAD, function () {
        it('loads spec and retains wildcard mark if the shelf has wildcard mark and keep wildcard mark is true', function () {
            var shelfSpec = spec_2.shelfSpecReducer(__assign({}, models_1.DEFAULT_SHELF_UNIT_SPEC, { mark: SHORT_WILDCARD }), {
        ...
        it('completely loads spec if the shelf has no wildcard mark', function () {
            var shelfSpec = spec_2.shelfSpecReducer(__assign({}, models_1.DEFAULT_SHELF_UNIT_SPEC, { mark: 'point' }), {
                type: spec_1.SPEC_LOAD,
                payload: {
                    spec: {
                        mark: 'bar',
