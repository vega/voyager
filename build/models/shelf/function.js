...
var AGGREGATE_INDEX = vega_util_1.toSet(aggregate_1.AGGREGATE_OPS);
var TIMEUNIT_INDEX = vega_util_1.toSet(timeunit_1.TIMEUNITS);
...
function fromFieldQueryFunctionMixins(fieldQParts) {
    ...
    if (bin) {
        if (wildcard_1.isWildcard(bin)) {
            var bins = wildcard_1.isShortWildcard(bin) ? [false] : bin.enum;
...
function sortFunctions(fns) {
    ...
    return fns.map(function (f) { return f || null; })
        .sort(function (a, b) { return FUNCTIONS_INDEX[a] - FUNCTIONS_INDEX[b]; })
